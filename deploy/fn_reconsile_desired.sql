...
*/
DROP FUNCTION IF EXISTS public.resonsile_desired(character varying, character varying, character varying, character varying);
DROP FUNCTION IF EXISTS public.reconsile_desired(og_schema_name character varying, ds_schema_name character varying, object_name character varying, dry_run boolean);

CREATE FUNCTION public.reconsile_desired(
    og_schema_name character varying, ds_schema_name character varying,
    object_name character varying, dry_run boolean)
...
END
$BODY$
    LANGUAGE plpgsql VOLATILE -- necessary?
    COST 100;                 -- necessary?


drop table if exists public.test_reconsile, desired_test_reconsile;
create schema if not exists desired;
create table if not exists public.test_reconsile(a int, b text, c int, d int, e int);
create table if not exists desired.test_reconsile(a int, b text, g int);

SELECT *
FROM public.reconsile_desired(character varying 'public',
                              character varying 'desired',
                              character varying'test_reconsile',
                              0::boolean);
